...
def index(request):
    ...
    context = {
        ...
        'latest_document_relation_list': DocumentRelation.objects.filter(published=True).filter(relation_type='A').order_by('-date_added')[0:5],
...
def project_part(request, project_part_id):
    ...
    context = RequestContext(request, {
        ...
        'doc_content_relation_list': project_part.document_relations.filter(published=True).filter(relation_type='C'),
        'doc_annotation_relation_list': project_part.document_relations.filter(published=True).filter(relation_type='A'),
...
def event(request, event_id):
    ...
    context = RequestContext(request, {
        ...
        'doc_content_relation_list': event.document_relations.filter(published=True).filter(relation_type='C'),
        'doc_annotation_relation_list': event.document_relations.filter(published=True).filter(relation_type='A'),
        'comment_form': comment_form,
        'comment_form_valid': comment_form_valid,
...
def participant(request, participant_id):
    ...
    context = RequestContext(request, {
        ...
        'doc_content_relation_list': participant.document_relations.filter(published=True).filter(relation_type='C'),
        'doc_annotation_relation_list': participant.document_relations.filter(published=True).filter(relation_type='A'),
        'comment_form': comment_form,
        'comment_form_valid': comment_form_valid,
